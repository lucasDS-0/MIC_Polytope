...
for n in range(MIN_NODOS, MAX_NODOS):
    for i in range(0, CANT_INSTANCIAS_N):
        ...
        mGdenso = np.random.randint(n*n/4,n*(n-1)/2,size=1)[0]
        mH = np.random.randint(1,n*(n-1)/2, size=1)[0]
        ...
        filas_indice.append([ n, mGdenso, mH, F"denso/instancia-{n}-nodos-Gdenso-{i}.txt"])
