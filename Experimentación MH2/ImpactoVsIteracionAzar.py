...
INSTANCIAS_PATH_IN = 'instancias/impactoVsIteracion/68/*'
...
def run(metodo, parametros, tipoVecindad):
    ...
    for instancia_path in glob.glob(INSTANCIAS_PATH_IN):
        ...
        for i in range(0,cantidad_corridas):
            ...
            for j in range (0, len(impactos_iteraciones)):
                filas.append([metodo, data[0], data[1], data[2], j, impactos_iteraciones[j], tipoVecindad]);
...
for i in {0,2}:
    parametros =f"0 1 0 1 2 50 0 -1 -1"
