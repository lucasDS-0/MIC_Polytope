...
NOMBRE_EXPERIMENTO = 'vecindad-size-ralo-2'
MODIFIED_PATH = './'+ NOMBRE_EXPERIMENTO +'/'
OUTPUT_PATH = MODIFIED_PATH + 'output/'
INSTANCIAS_PATH_IN = 'instancias/ralo/*'
...
def run(metodo, parametros, vecindadSize):
    ...
    for instancia_path in glob.glob(INSTANCIAS_PATH_IN):
        ...
        filas.append([metodo, data[0], data[1], mH, impacto_promedio, tiempo, error, vecindadSize,group]);
...
for i in {-2,-1,1}:
    parametros =f"0 1 0 1 2 50 {i} -1 -1"
