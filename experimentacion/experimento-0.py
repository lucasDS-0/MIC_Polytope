...
NOMBRE_EXPERIMENTO = 'mem-size-ralo'
...
def run(metodo, parametros, memsize):
    ...
    for instancia_path in glob.glob(INSTANCIAS_PATH_IN):
        ...
        mH = mH.split("\n")[0]
        mH =float(mH)
        error=( mH - impacto_promedio)/mH

        filas.append([metodo, data[0], data[1], data[2], impacto_promedio, tiempo, error, memsize]);
...
for i in range(1,4):
    p = i*25
    parametros =f"0 1 0 2 0 {p} 0 -1 -1"
