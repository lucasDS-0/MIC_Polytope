...
NOMBRE_EXPERIMENTO = 'comportamiento_vecindad'
...
def correr_experimento(metodo, parametros, instancia,archivo_instancia):
    ...
    process = subprocess.Popen(["../build/tp2", metodo], stderr=subprocess.PIPE, stdout=subprocess.PIPE, stdin=subprocess.PIPE, universal_newlines = True)
...
def run(metodo, parametros, colores, factor):
    ...
    for instancia_path in glob.glob(INSTANCIAS_PATH_IN):
        ...
        filas.append([metodo, data[0], data[1], data[2], impacto_promedio, tiempo, error, colores, factor]);
...
for i in range(0,3,2):   
    for j in range(0,2):
        parametros =f"0 1 2 {i} -1 -1 -1 1 {j}"
